...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), |worker| {
        ...
        let exchange = ExchangeCore::new(|x: &(&str, i64)| (x.0).len() as u64);

        // create a new input, exchange data, and inspect its output
        worker.dataflow::<usize,_,_>(|scope| {
            input.to_stream(scope)
                 .flat_map::<FlatStack<<(String, i64) as Containerized>::Region>, _, _>(|(text, diff): (String, i64)|
                    text.split_whitespace()
                        .map(move |word| (word.to_owned(), diff))
                        .collect::<Vec<_>>()
                 )
                 .unary_frontier::<FlatStack<_>, _, _, _>(exchange, "WordCount", |_capability, _info| {
                    ...
                    move |input, output| {
                        while let Some((time, data)) = input.next() {
                            queues.entry(time.retain())
                                  .or_insert(Vec::new())
                                  .push(data.replace(FlatStack::default()));
                        }
                            ...
                            if !input.frontier().less_equal(key.time()) {
                                ...
                                for mut batch in val.drain(..) {
                                    for (word, diff) in batch.iter() {
                                        let entry = counts.entry(word.to_string()).or_insert(0i64);
                                        *entry += diff;
