...
// We cannot derive `Debug` because we don't want to force `D::Allocation: Debug`.
impl<T: Timestamp, D: Container> ::std::fmt::Debug for UnorderedHandleCore<T, D> where D: ::std::fmt::Debug {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut debug = f.debug_struct("UnorderedHandleCore");
