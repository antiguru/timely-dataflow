...
pub trait Inspect<G: Scope, D, C: Container = Vec<D>>: InspectCore<G, C> + Sized {
    ...
    /// });
    /// ```
    fn inspect_batch(&self, mut func: impl FnMut(&G::Timestamp, &[D])+'static) -> Self {
    ...
    /// });
    /// ```
    fn inspect_core<F>(&self, func: F) -> Self where F: FnMut(Result<(&G::Timestamp, &[D]), &[G::Timestamp]>)+'static;
}

impl<G: Scope, D: Data> Inspect<G, D> for StreamCore<G, Vec<D>> {
...
impl<G: Scope, D: Data+Columnation> Inspect<G, D, TimelyStack<D>> for StreamCore<G, TimelyStack<D>> {
    fn inspect_core<F>(&self, mut func: F) -> Self where F: FnMut(Result<(&G::Timestamp, &[D]), &[G::Timestamp]>) + 'static {
        self.inspect_container(move |r| func(r.map(|(t, c)| (t, &c[..]))))
    }
}

impl<G: Scope, D, C: Container> Inspect<G, D, Rc<C>> for StreamCore<G, Rc<C>>
    where C: AsRef<[D]>
{
    fn inspect_core<F>(&self, mut func: F) -> Self where F: FnMut(Result<(&G::Timestamp, &[D]), &[G::Timestamp]>) + 'static {
